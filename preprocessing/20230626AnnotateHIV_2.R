...
#this is the same as the 6/19 attempt, but it includes only ATAC reads from autologous and HXB2. 
#We avoid the autologous because it's clear that there is nosie there we can't easily account for due to the annotation of non CD4 cells as infected in large numbers
...
todo<-names(meta)
final<-rbind(ProcessHIVAnnotation(paste0("~/gibbs/DOGMAMORPH/data/HIV_RNA2/", todo[[1]],".results.txt" ), paste0("~/gibbs/DOGMAMORPH/data/HIV_ATAC/indexreads/", todo[[1]]), prefix = todo[[1]], bcs)
             , ProcessHIVAnnotation(paste0("~/gibbs/DOGMAMORPH/data/HIV_RNA2/", todo[[2]],".results.txt" ), paste0("~/gibbs/DOGMAMORPH/data/HIV_ATAC/indexreads/", todo[[2]]), prefix = todo[[2]], bcs))
for(i in 3:length(todo)){
  final<-rbind(final, ProcessHIVAnnotation(paste0("~/gibbs/DOGMAMORPH/data/HIV_RNA2/", todo[[i]],".results.txt" ), paste0("~/gibbs/DOGMAMORPH/data/HIV_ATAC/indexreads/", todo[[i]]), prefix = todo[[i]], bcs))
}
...
saveRDS(results, "gibbs/DOGMAMORPH/Ranalysis/Objects/20230626FinalClusternames.rds")

table(results$HIV_reads_ATAC)
table(results$HIV_reads_RNA)
table(results$HIV_reads_RNA, results$HIV_reads_ATAC)
#17 RNA cells
sum(results$HIV_reads_RNA>1)
#26 ATAC cells
sum(results$HIV_reads_ATAC>1)
#38 cells total
...
marks_RNA<-FindMarkers(results, TRUE, FALSE)
Idents(results)<-(results$HIV_reads_RNA>1)|(results$HIV_reads_ATAC>1)
marks_either<-FindMarkers(results, TRUE, FALSE)
